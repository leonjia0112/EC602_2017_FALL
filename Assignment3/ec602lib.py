...
def get_includes(file_contents):
    ...
    for line in file_contents.lower().splitlines():
        text = line.strip()
        matches = re.match("#include\s*<(.*)>", text)
        if matches:
...
def overallcpp(program_name,
    ...
    retstr += "lines of code : {}, {:4.0%} of reference\n".format(
        code_metrics['lines'], code_metrics['lines'] / refcode['lines'])
    retstr += "tokens in code: {}, {:4.0%} of reference\n".format(
        code_metrics['words'], code_metrics['words'] / refcode['words'])
    retstr += "comments      : {}\n".format(comments)
